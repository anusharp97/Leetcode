...
the numeric value of c is 3, and so on. The numeric value of a string consisting of lowercase characters is defined as the sum of its characters' numeric values. 
For example, the numeric value of the string "abe" is equal to 1 + 2 + 5 = 8. You are given two integers n and k. Return the lexicographically smallest string with 
...
def getSmallestString(self, n: int, k: int) -> str:
  ...
  for i in range(26,0,-1):
      letterCount = k//i
      if letterCount>0:
          if k-(letterCount*i)>=n-letterCount:
              res += letterCount*(chr(i+96))
              k = k - (letterCount*i)
              n -= letterCount
          else:
              j = 1
